...
class SystemState:
    ...
    def get_camera(self):
        ...
        if self.camera is None or not self.camera.isOpened():
            self.camera = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_DSHOW)  # DirectShow backend
...
def generate_frames():
    ...
    while True:
        try:
            ...
            if not success:
                # Camera read failed after retries, try reconnect
            ...
            if frame_count % 3 == 0:
                ...
                # Use previous detection results for non-processed frames
                with state.frame_lock:
                    if state.latest_frame is None:
                        annotated = frame
                    else:
                        # Just use the frame without heavy processing
                        annotated = frame
